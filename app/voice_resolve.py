...
def _build_voice_prompt(known_antibiotics):
    ...
    return (
        ...
        "Listen to the ENTIRE clip carefully before deciding, all the way to the end -- do not commit "
        "to an answer from only the first syllable or two. If the word is spoken more than once in the "
        "clip (people often repeat a drug name for clarity, or the recording starts a beat late), use "
        "whichever repetition came through most clearly, and treat the other repetitions as supporting "
        "evidence for the same answer rather than separate guesses. Background noise, a quiet/muffled "
        "recording, or a strong accent are normal here, not reasons to give up -- work with whatever "
        "signal is present the way an experienced pharmacist listening over a noisy phone line would: "
        "focus on the number of syllables, the stressed syllable, and the consonant sounds that survive "
        "noise best (the drug-class-defining parts like '-cillin', '-mycin', '-floxacin', '-azole', "
        "'ceph-/cef-', 'peni-'), since vowels and soft consonants are what background noise usually "
        "destroys first.\n\n"
        ...
        "match, not just transcribing literally. Prefer a confident best real-drug guess over an early "
        "'unmatched' verdict -- this hospital's pharmacist can always overrule an incorrect guess by "
        "typing the correct name by hand, but a guess you never made can't be corrected, so lean toward "
        "identifying a specific real antibiotic whenever the sounds plausibly support one.\n\n"
        ...
        "any specific real antibiotic from the audio (silence, unrelated speech, or a word that plainly "
        "doesn't correspond to any real drug name even after considering accent and noise) -- in that "
        "case put your best plain transcription in both 'heard' and 'resolved_name' rather than "
        "guessing a random drug."
    )
